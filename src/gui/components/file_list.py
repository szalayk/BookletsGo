...
class FileListPanel(ctk.CTkFrame):
    def __init__(self, parent):
        super().__init__(
            parent, fg_color=Styles.BG_PANEL, corner_radius=Styles.CORNER_RADIUS
        )
    ...
    def _build_header(self):
        ...
        header_frame.grid(
            row=0, column=0, sticky="ew", padx=Styles.PADDING_MD, pady=Styles.PADDING_MD
        )
    ...
    def _build_scroll_area(self):
        ...
        self.scroll_frame.grid(
            ...
            column=0,
            sticky="nsew",
            padx=Styles.PADDING_MD,
            pady=(0, Styles.PADDING_MD),
        )
    ...
    def _on_add_files_click(self):
        ...
        selected_paths = filedialog.askopenfilenames(
            title="Select Files to Include in Booklet", filetypes=file_types
        )
    ...
    def _render_file_row(self, index, filepath):
        ...
        row_frame = ctk.CTkFrame(
            self.scroll_frame, fg_color=row_bg, corner_radius=Styles.CORNER_RADIUS
        )
        ...
        file_lbl.grid(row=0, column=0, sticky="w", padx=Styles.PADDING_SM, pady=6)
        ...
        if index == len(self.files) - 1:
            down_btn.configure(state="disabled", text_color="#B3B3B3")
    ...
    def _move_item(self, index, direction):
        ...
        if 0 <= target_index < len(self.files):
            self.files[index], self.files[target_index] = (
                self.files[target_index],
                self.files[index],
            )
            self._refresh_list()
