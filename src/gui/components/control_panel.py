...
class ControlPanel(ctk.CTkFrame):
    def __init__(self, parent, on_generate_callback):
        ...
        self.grid_rowconfigure(
            1, weight=1
        )  # Options Box (stretches to push button down)
    ...
    def _build_options(self):
        ...
        hint_lbl = ctk.CTkLabel(
            ...
            wraplength=200,  # Ensure it wraps cleanly inside the narrow panel
            justify="left",
        )
        hint_lbl.grid(row=1, column=0, sticky="w", padx=(30, 0))
    ...
    def _build_action_button(self):
        ...
        self.generate_btn.grid(
            row=2, column=0, sticky="ew", padx=Styles.PADDING_MD, pady=Styles.PADDING_MD
        )

    def _on_generate_click(self):
        """Internal event handler that collects options and fires the callback."""
        options = {"numbering": self.numbering_var.get()}
        if self.on_generate:
